...
if __name__ == "__main__":
    try:
        init_data1 = []
        canid1, mstid1 = 0x01, 0x11
        canid2, mstid2 = 0x02, 0x12
        canid3, mstid3 = 0x03, 0x13
        canid4, mstid4 = 0x04, 0x14
        canid5, mstid5 = 0x05, 0x15
        canid6, mstid6 = 0x06, 0x16
        canid7, mstid7 = 0x07, 0x17
        ...
        with Motor_Control(1000000, 5000000, "2EBF423413AA04B9E80688FE6504D508", init_data1, device_type=dmcan_device_type.USB2CANFD) as control:
            while running.is_set():
                desired_duration = 0.001
                current_time = time.perf_counter()

                control.control_mit(control.getMotor(canid1), 0.5, 0.5, 5.0, 1.0, 0.0)
                control.control_mit(control.getMotor(canid2), 2.0, 0.5, 2.0, 1.0, 0.0)
                control.control_mit(control.getMotor(canid3), 2.0, 0.5, 2.0, 1.0, 0.0)
                control.control_mit(control.getMotor(canid4), 2.0, 0.5, 2.0, 1.0, 0.0)
                control.control_mit(control.getMotor(canid5), 2.0, 0.5, 2.0, 1.0, 0.0)
                control.control_mit(control.getMotor(canid6), 2.0, 0.5, 2.0, 1.0, 0.0)
                control.control_mit(control.getMotor(canid7), 2.0, 0.5, 2.0, 1.0, 0.0)
                ...
                #     print(f"canid is: {id} pos: {pos:.3f} vel: {vel:.3f} "
                #           f"effort: {tau:.3f} err: {err} time(s): {interval:.4f}",
                #           file=sys.stderr)
                # print(
